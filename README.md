...
Load order: `db.lua` → `Gear.lua` → `Main.lua` → `UI.lua`

## Data & migration

Saved data uses `LoadoutLockerDB` (per character).

If you previously used **GearSpecChanger**, existing data in `GearSpecChangerDB` is migrated automatically on first load.
